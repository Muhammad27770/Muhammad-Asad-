...
# @app.route('/library/account', methods=['POST'])
# def create_library_account():
#     data = request.json
#     student_id = str(data['id'])
#     if db.execute('SELECT * FROM student WHERE id = %s', student_id):
#         return {'studentId': student_id, 'message': 'Student already registered'}, 200
#     db.execute('INSERT INTO student(id, hash) VALUES (%s, %s)',
#                student_id, generate_password_hash(constants.DEFAULT_PIN))
#     return {'studentId': student_id, 'message': 'Library account created successfully'}, 201
